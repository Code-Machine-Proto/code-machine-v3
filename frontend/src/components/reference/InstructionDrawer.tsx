...
export default function InstructionDrawer(props: Props) {
  ...
  return (
    <div
      ...
      classList={{ 'pointer-events-none opacity-0': !props.open(), 'pointer-events-auto opacity-100': props.open() }}
    >
      {/* Backdrop */}
      ...
      >
        ...
        <div class="panel-header py-3">
          ...
          <button onClick={props.onClose} class="btn-control w-6 h-6 text-xs">
                  ...
                  <For each={instrs}>
                    {(instr) => (
                      <div class="flex items-baseline gap-3 px-2 py-1.5 rounded-md hover:bg-main-900 transition-colors group">
                        <code class="text-accent font-mono font-semibold text-sm w-12 shrink-0">{instr.mnemonic}</code>
                        <span class="text-main-500 font-mono text-xs w-24 shrink-0">{instr.operands}</span>
                        <span class="text-main-400 text-xs">{instr.description}</span>
                      </div>
                    )}
                  </For>
                </div>
        ...
        <div class="shrink-0 px-3 py-2 border-t border-main-700/50">
          <p class="text-main-600 text-[10px] text-center">{instructions().length} instructions disponibles</p>
        </div>
      </div>
    </div>
  );
