    ...
    <div class="flex items-center gap-3 px-4 py-1.5 bg-main-900">
      ...
      <div class="flex items-center gap-1">
        ...
        <ControlButton
          ...
          disabled={() => !props.isCompiled() || props.currentStep() <= 0}
          title="Pas precedent"
        >
          <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <ControlButton
          ...
          disabled={() => !props.isCompiled() || props.currentStep() >= props.totalSteps() - 1}
          title="Pas suivant"
        >
          <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
...
function ControlButton(props: { onClick: () => void; disabled: () => boolean; title: string; children: JSX.Element }) {
  return (
    <button onClick={props.onClick} disabled={props.disabled()} class="btn-control w-8 h-8" title={props.title}>
      {props.children}
    </button>
  );
}
