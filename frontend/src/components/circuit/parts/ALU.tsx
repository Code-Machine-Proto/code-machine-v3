  ...
  return (
    <svg x={props.x} y={props.y} width="70" height="170" viewBox="0 0 170 422" fill="none">
      <path
        ...
        stroke-width="2"
      />
      <text x={12} y={95} class="font-semibold text-xl" fill={labelFill()}>
        A
      </text>
      <text x={12} y={285} class="font-semibold text-xl" fill={labelFill()}>
        B
      </text>
      <text x={158} y={190} text-anchor="end" class="font-semibold text-xl" fill={labelFill()}>
        F
      </text>
      <text x={158} y={70} text-anchor="end" class="font-bold text-xl" fill={labelFill()}>
        ALU
      </text>
      <path d="M40 368L25.5662 393H54.4338L40 368ZM40 422H42.5L42.5 390.5H40H37.5L37.5 422H40Z" class={arrowClass()} />
      <text x={50} y={410} class="text-base circuit-label">
        op_alu
      </text>
      {props.hasNz && (
        <>
          <path
            ...
            class={arrowClass()}
          />
          <text x={130} y={400} class="text-base circuit-label">
            NZ
          </text>
        </>
      )}
