...
import { themeCompartment, highlightCompartment, getThemeExtension, getHighlightExtension } from './theme';
  ...
  if (window.showSaveFilePicker) {
    try {
      const handle = await window.showSaveFilePicker({
        suggestedName,
        types: [{ description: 'Fichier assembleur', accept: { 'text/plain': ['.s', '.asm', '.txt'] } }],
      });
  ...
  update(decorations, tr) {
    ...
    for (const effect of tr.effects) {
      ...
      if (lineNumber === null || lineNumber < 0 || lineNumber >= tr.state.doc.lines) {
        ...
        decorations = Decoration.set([Decoration.line({ class: 'cm-active-exec-line' }).range(line.from)]);
      }
    }
    return decorations;
  },
...
export default function CodeEditor(props: Props) {
  ...
  const handleSave = () => {
    if (!view) return;
    void saveCodeToFile(view.state.doc.toString(), defaultFileNames[props.processorId]);
  };
  ...
  return (
    <div class="flex flex-col h-full">
      ...
      >
        <div class="panel-header gap-2">
          <button
            ...
            title="Masquer l'editeur"
          >
            <svg class="w-2.5 h-2.5 -rotate-90" fill="currentColor" viewBox="0 0 20 20">
              <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
            </svg>
          </button>
          <span class="panel-label shrink-0">Éditeur</span>
          <input
            ref={fileInputRef}
            type="file"
          ...
          <span
            ...
            classList={{
              'bg-main-800 text-main-600':
                !props.isCompiling() && !props.isStale() && errorCount() === 0 && !props.isCompiled(),
              'bg-accent/15 text-accent-light': props.isCompiling(),
              'bg-amber-500/15 text-amber-400': !props.isCompiling() && props.isStale(),
              'bg-red-500/15 text-red-400': !props.isCompiling() && !props.isStale() && errorCount() > 0,
          ...
          >
                    ...
                    <Show
                      ...
                      fallback={
                        <Show
                          ...
                          fallback={
                            <svg class="w-2 h-2 shrink-0" fill="currentColor" viewBox="0 0 24 24">
            ...
            >
              <svg class="w-3 h-3 shrink-0 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
      ...
      <div ref={containerRef} class="flex-1 overflow-auto" classList={{ hidden: props.collapsed }} />
    </div>
  );
}
